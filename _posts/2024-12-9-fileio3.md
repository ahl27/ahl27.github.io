...
A *loser tree* is a type of tournament tree in which the *loser* advances to the next round, rather than the winner. In code, the "game" played is which value is larger. This means that the larger value will stay at the internal nodes, and the smaller value will continue on up the tree. Eventually, the smallest value from the leaves ends up at the top of the tree.
...
- B vs. A results in B winning and A continuing on
- A vs. C results in A winning and C continuing on
...
This seems like more steps, but it actually isn't. When we move down the tree in the heap approach, we have to make two comparisons at each node. Notice how we do two pairs of comparisons (8 vs 2,3; 8 vs 6,7) to update the heap in the above example. In contrast, the loser tree approach moves up the tree, so we only compare with the parent of the current node in each case. Thus, the tournament tree uses $\log k$ comparisons to update a value in the tree, whereas a heap uses $2\log k$. If we're processing a lot of data, that difference will add up over time.

## Performance and Implementation

This post focuses on the conceptual implementation, but I'll also include the code below. One of the goals of this process was to speed up how fast we could read in edges from a `tsv` file. Here's how this looks in practice on a dataset with 74 million lines each encoding a bidirectional edge:

- Previous approach (write to random locations): 640 seconds
- External merge sort (no loser tree): 189 seconds (70.5% faster than naive)
- External Loser tree 8-way merge: **42 seconds** (93.4% faster than naive)

I agree with Bryan Boreham -- that's blazingly fast. The main remaining issue with this implementation is it doubles our disk space consumption, since I'm sorting by reading from one file and outputting to another. The next task will be implementing this strategy with an in-place sort, which is much more complicated than it might initially seem...but that's a task for another blog post.
