...
It turns out that `fseek` is *always* using `SEEK_SET` (or `SEEK_END`). When `fseek` is called, it first has to figure out the absolute position in the file to go to. When `whence` is `SEEK_CUR`, it has to do some additional calculations to figure out where the pointer *currently* is, which isn't as simple as it sounds due to potential internal buffering. In fact, when buffering is disabled on the HDD, you can see that the difference between `SEEK_CUR` and `SEEK_SET` for the second benchmark is much smaller than when buffering is enabled.
...
- SSDs have very good random access for reads. Regardless of parameterization, all my results in Benchamrk 1 were essentially identical on SSDs.
